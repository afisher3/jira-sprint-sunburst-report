...
Condensed refresher — read `CLAUDE.md` in full at least once; use this afterward to reorient cheaply. If something here and there disagree, `CLAUDE.md` wins.

**What this is:** Lambda pulls Jira Cloud sprint data → rolls story points by 2-level classification → computes throughput/return-rate/stage metrics → renders one self-contained HTML file (Plotly sunburst + tickets table) → uploads to S3.
...
**Ticket table filtering = by issue key, never by current status.** Every clickable card (`.stat-card-clickable` + `data-throughput-key`) — Throughput (`refinement`/`dev`/`qa`/`uatSignoff`) and Return Rates (`qaReturn`/`uatReturn`) alike — filters via `throughputIssueKeys[sprintId][key]`. Adding a new one: see `.claude/skills/add-filterable-stat-card`.
...
**Build/test:** see `.claude/skills/build-and-test` — `npm run build` then `npx vitest run` with `dangerouslyDisableSandbox: true` (native Rollup binary code-signing issue on this machine, not a real failure).
