...
The tickets table shows the full per-sprint issue list for the selected sprints. Clicking any clickable stat card — every card with class `stat-card-clickable` and a `data-throughput-key` attribute, which today spans the Throughput panel (`refinement`/`dev`/`qa`/`uatSignoff`), the Return Rates panel (`qaReturn`/`uatReturn`), and the Sprint Details panel (`stale`/`rollover`) — filters that table down to the issue **keys** returned by that stage's JQL query — **not** by each issue's current status. An issue's current status is only ever displayed as a column value, never used as a filter predicate. (An earlier design that filtered by current status was explicitly rejected — current status doesn't reflect "did this issue pass through this stage at some point," which is what these cards actually measure.) This mechanism is fully generic client-side: `getThroughputKeySet()`/`updateIssuesTable()`/the card click handler all key off `data-throughput-key` and the matching field on `throughputIssueKeys[sprintId]`, so a brand-new clickable card needs no new client-JS logic — see `add-filterable-stat-card` in `.claude/skills/`.

`stale` (open issues — excludes Resolved/Closed — with no status change in 14 days, also scoped by the `filter = 11682` saved filter used elsewhere in `issue-repository.ts`) is a per-sprint JQL query, same shape as the Throughput/Return Rate cards. `rollover` (issues present in > 2 of the windowed sprints) is different: it has no JQL of its own — `ReportGenerator` derives it in a post-pass over the already-fetched `issuesBySprint` (a cross-sprint property, not a per-sprint query), using the full loaded sprint window regardless of which sprints are later checked in the UI. Both thresholds (14 days, > 2 sprints) are hardcoded constants, not config, matching how the 30-day rolling window is also hardcoded. Their displayed counts use `getThroughputKeySet(...).size` (deduped) rather than summing per-sprint key-list lengths like the Throughput cards do — a rollover ticket by definition appears in multiple selected sprints' key lists, so summing lengths would multiply-count it.
